...
public class RunTimeEscVerificationCodeGenerator extends JmlTreeScanner {
    ...
    public RunTimeEscVerificationCodeGenerator(
    		Context context,
        ...
        for (JmlSpecificationCase c: tree.cases) {
            ...
            for (JmlMethodClause clause: c.clauses) {
                if (
                    ...
                    GetNames(expr.expression, used_vairables);
    ...
    public void visitJmlMethodSpecs(JmlMethodSpecs tree) {
        ...
        if (
        	//Check for null
        	tree == null || tree.decl == null || tree.decl.name == null || tree.decl.params == null
        	|| (
        		// Check values
        ...
        /*Testing
        var block = maker.Block(0L, List.from(CompileTimeEscVerificationCodeGenerator.instance.GetSpecStatements(tree, null)));
        var methodBody = ((JCTree.JCMethodDecl) tree.decl).body;
        methodBody.stats = methodBody.stats.prepend(block);
