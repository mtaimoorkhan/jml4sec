...
import static org.jmlspecs.openjml.ext.ActionClauseExtension.actionClauseKind;
import static org.jmlspecs.openjml.ext.ActionClauseExtension.actionID;
import static org.jmlspecs.openjml.ext.AlarmsClauseExtension.alarmsClauseKind;
import static org.jmlspecs.openjml.ext.AlarmsClauseExtension.alarmsID;
...
public class JmlAttr extends Attr implements IJmlVisitor {
    ...
    public static JmlAttr instance(Context context) {
    	JmlAttr instance = (JmlAttr) context.get(attrKey);
        if (instance == null)
            instance = new JmlAttr(context); // Registers itself in the super constructor
        return instance; // If the registered instance is only an Attr, something is catastrophically wrong
...
//    	this.env = prevEnv;
    ...
    final static Map<IJmlClauseKind, int[]> stateTable = new HashMap<>();
    {
        ...
        stateTable.put(actionClauseKind, new int[]{1,1,1});
        //ADD-END*/
    ...
    protected void handleCase(JmlSpecificationCase parent, JmlMethodDecl decl, MethodSymbol msym,
        ...
        if (parent == null) {
            ...
            //ADD-OPENJMLSEC(Wyatt)-RUN
	            ...
	            JmlMethodClauseExpr cl = jmlF.at(cse.pos).JmlMethodClauseExpr(alarmsID, alarmsClauseKind,falseLit);
	            cl.sourcefile = cse.sourcefile;
	            pr.append(cl);
	        } else if (t == actionBehaviorClause) {
	            JmlMethodClauseExpr cl = jmlF.at(cse.pos).JmlMethodClauseExpr(actionID, actionClauseKind,falseLit);
	            cl.sourcefile = cse.sourcefile;
	            pr.append(cl);
	        }
            //ADD-END*/
...
//ADD-OPENJMLSEC(Wyatt)-RUN
                if (parent.token == compromisedBehaviorClause) {
                    log.error(m.pos,"jml.misplaced.clause","Ensures","compromised");
                    continue;
                }
//ADD-END*/
...
//ADD-OPENJMLSEC(Wyatt)-RUN
                if (parent.token == compromisedBehaviorClause) {
                    log.error(m.pos,"jml.misplaced.clause","Signals","compromised");
                    continue;
                }

            } else if (t == alarmsClauseKind) {
                if (parent.token == normalBehaviorClause || parent.token == normalExampleClause) {
                    log.error(m.pos, "jml.misplaced.clause", "Alarms", "normal");
                    continue;
                }
                if (parent.token == exceptionalBehaviorClause || parent.token == exceptionalExampleClause) {
                    log.error(m.pos, "jml.misplaced.clause", "Alarms", "exceptional");
                    continue;
                }
                if (parent.token == actionBehaviorClause) {
                    log.error(m.pos, "jml.misplaced.clause", "Alarms", "action");
                    continue;
                }
            } else if (t == actionClauseKind) {
                if (parent.token == normalBehaviorClause || parent.token == normalExampleClause) {
                    log.error(m.pos, "jml.misplaced.clause", "Action", "normal");
                    continue;
                }
                if (parent.token == exceptionalBehaviorClause || parent.token == exceptionalExampleClause) {
                    log.error(m.pos, "jml.misplaced.clause", "Action", "exceptional");
                    continue;
                }
//ADD-END*/
...
//ADD-OPENJMLSEC(Wyatt)-RUN
                if (parent.token == compromisedBehaviorClause) {
                    log.error(m.pos,"jml.misplaced.clause","Signals_only","compromised");
                    continue;
                }
//ADD-END*/
                int count = 0;
...
//        }
    ...
    @Override
    public void visitJmlMethodClauseAction(JmlMethodClauseAction tree) {
        //attribExpr(tree.expression, env, syms.booleanType);
    }
    //ADD-END
        ...
        if (jmlVisibility != -1) {
            ...
            if (jmlenv.currentClauseKind == invariantClause || jmlenv.currentClauseKind == constraintClause) {
                ...
                if (!moreOrEqualVisibleThan(v,jmlVisibility)) {
            		...
            		//ADD-OPENJMLSEC(Wyatt)-RUN
                    || jmlenv.currentClauseKind == alarmsClauseKind
                    || jmlenv.currentClauseKind == actionClauseKind
            		//ADD-END*/
            		) {
...
//            }
    ...
    public boolean interpretInPreState(DiagnosticPosition pos, IJmlClauseKind kind) {
        if (kind == ensuresClauseKind ||
                ...
                kind == signalsOnlyClauseKind
                //ADD-OPENJMLSEC(Wyatt)-RUN
                || kind == alarmsClauseKind
                || kind == actionClauseKind
                //ADD-END*/
        		) return false;
