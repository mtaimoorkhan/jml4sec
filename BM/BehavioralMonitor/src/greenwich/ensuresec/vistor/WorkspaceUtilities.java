...
public class WorkspaceUtilities {
	...
	public static List<ICompilationUnit> collectCompilationUnits(IJavaElement javaElement) {
		...
		// We are traversing the JavaModel for COMPILATION_UNITs
 		...
 		//	parseProject(javaElement.getPath());
...
/*	
public static void parseProject(IPath iPath) {
			
		
		Path source=Paths.get("/Users/drijazahmed/runtime-EclipseApplication/BehMonitorTests/src/tests");
			
	        SourceRoot root = new SourceRoot(source);
	        try {
	            ...
	            System.out.println("parse root {} error {}");
	        }
